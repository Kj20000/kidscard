...
// Initialize all stores by accessing them once
// This ensures all object stores are created in the database
async function initializeDatabase(): Promise<void> {
  if (dbInitialized) return;
  
  try {
    // Access each store to ensure they're all created
    const stores = [flashcardsStore, categoriesStore, imagesStore, settingsStore];
    await Promise.all(stores.map(store => keys(store).catch(() => [])));
    dbInitialized = true;
  } catch (error) {
    console.error('Failed to initialize database, resetting:', error);
    // If initialization fails, delete and recreate the database
    try {
      const DBDeleteRequest = indexedDB.deleteDatabase('flashcards-db');
      await new Promise((resolve, reject) => {
        DBDeleteRequest.onsuccess = resolve;
        DBDeleteRequest.onerror = reject;
      });
      // Try initialization again after deletion
      const stores = [flashcardsStore, categoriesStore, imagesStore, settingsStore];
      await Promise.all(stores.map(store => keys(store).catch(() => [])));
      dbInitialized = true;
    } catch (resetError) {
