...
export const ENABLE_CLOUD_SYNC = true;
 ...
 export function useFlashcardSync() {
   ...
   // Set up real-time subscription to listen for changes from other devices
   useEffect(() => {
     if (!ENABLE_CLOUD_SYNC) return;

     let categoryChannel: any;
     let flashcardChannel: any;
     ...
     const setupRealtimeSubscriptions = async () => {
       ...
       categoryChannel = supabase
         ...
         .on(
           ...
           async (payload) => {
             ...
             // Pull latest data when changes are detected
             await pullFromCloud();
           }
         )
         .subscribe();
       ...
       flashcardChannel = supabase
         ...
         .on(
           ...
           async (payload) => {
             console.log('📡 Real-time flashcard change detected:', payload);
             // Pull latest data when changes are detected
             await pullFromCloud();
           }
         )
  ...
  const syncToCloud = useCallback(async (): Promise<SyncResult> => {
    ...
    try {
      ...
      // Sync ALL categories first (flashcards depend on them)
      // This ensures categories exist in DB even if marked as 'synced' locally
      if (allCategories.length > 0) {
        const categoriesToSync = allCategories.map(cat => ({
          id: cat.id,
          user_id: user.id,
          name: cat.name,
          icon: cat.icon,
      ...
      return {
        ...
        syncedCategories: allCategories.length,
      };
    } catch (error) {
  ...
  const pullFromCloud = useCallback(async (onDataUpdated?: () => void): Promise<SyncResult> => {
    if (!ENABLE_CLOUD_SYNC) {
      return { success: true, error: 'Cloud sync is disabled' };
    }
    ...
    try {
      ...
      // Notify caller that data was updated
      if (onDataUpdated) {
        onDataUpdated();
      }
